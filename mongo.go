...
func (c *Client) Find(database string, collection string, filter interface{}, projection interface{},sort interface{}, limit int64) ([]bson.M, error) {
	db := c.client.Database(database)
	col := db.Collection(collection)
	opts := options.Find().SetSort(sort).SetProjection(projection).SetLimit(limit)
